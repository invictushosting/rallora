-- RALLORA STAGING ONLY: core tenant RLS replacement; NEVER deploy directly to live project.
-- Depends on scripts/staging/001_club_scope_helpers.sql.
-- Replaces legacy global-admin policies on SIX tables. Before production:
-- backup, restore rehearsal, ALL adjacent tenant tables and all SECURITY DEFINER RPCs
-- need separate approval. Do NOT add this to supabase/migrations yet.
-- This policy set is deliberately tested against two synthetic clubs in CI.
...
-- Remove the earlier synthetic team policy if this script runs in CI.
