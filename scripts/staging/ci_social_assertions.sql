...
begin
  if (select count(*) from public.rallora_social_posts) <> 1 then
    raise exception 'Second A administrator cannot read own club drafts';
  end if;
  update public.rallora_social_posts set title='Hijacked A draft'
    where id='00000000-0000-0000-0000-0000000007a1';
  get diagnostics n = row_count;
  if n <> 0 then raise exception 'Non-author edited another administrator draft'; end if;
  delete from public.rallora_social_post_targets
    where id='00000000-0000-0000-0000-0000000008a1';
  get diagnostics n = row_count;
  if n <> 0 then raise exception 'Non-author deleted another administrator target'; end if;
  delete from public.rallora_social_posts
    where id='00000000-0000-0000-0000-0000000007a1';
  get diagnostics n = row_count;
