...
for select to anon using (is_published and exists (
  select 1 from public.clubs c where c.id=public.announcements.club_id and c.is_active
));
create policy "tenant_announcements_authenticated_read" on public.announcements
for select to authenticated using (
  (is_published and exists (select 1 from public.clubs c where c.id=public.announcements.club_id and c.is_active))
...
for select to anon using (is_active and exists (
  select 1 from public.clubs c where c.id=public.sponsors.club_id and c.is_active
));
create policy "tenant_sponsors_authenticated_read" on public.sponsors
for select to authenticated using (
  (is_active and exists (select 1 from public.clubs c where c.id=public.sponsors.club_id and c.is_active))
  or public.rallora_can_manage_club(club_id)
);
...
for select to anon using (
  exists (select 1 from public.clubs c where c.id=public.club_rules.club_id and c.is_active)
...
for select to anon using (
  exists (select 1 from public.clubs c where c.id=public.club_setup_profiles.club_id and c.is_active)
  and (season_id is null or exists (
...
for select to anon using (exists (
  ...
  where s.id=public.standings.season_id
    and d.id=public.standings.division_id
    and t.id=public.standings.team_id
...
for insert to authenticated with check (
  public.rallora_can_manage_season(season_id) and exists (
    ...
    where d.id=public.standings.division_id
      and d.season_id=public.standings.season_id
      and t.id=public.standings.team_id
  )
);
...
for select to anon using (is_active and exists (
  ...
  join public.clubs c on c.id=s.club_id
  where d.id=public.cup_qualifier_rules.division_id
    and s.id=public.cup_qualifier_rules.season_id
    and s.status in ('active','completed') and c.is_active
));
...
for insert to authenticated with check (
  ...
  and exists (select 1 from public.divisions d
    where d.id=public.cup_qualifier_rules.division_id
      and d.season_id=public.cup_qualifier_rules.season_id)
);
...
for select to anon using (is_active and exists (
  ...
  join public.seasons s on s.id=d.season_id
  join public.clubs c on c.id=s.club_id
  where t.id=public.cup_manual_qualifiers.team_id
    and s.id=public.cup_manual_qualifiers.season_id
    and s.status in ('active','completed') and c.is_active
));
...
for insert to authenticated with check (
  public.rallora_can_manage_season(season_id) and exists (
    ...
    where t.id=public.cup_manual_qualifiers.team_id
      and d.season_id=public.cup_manual_qualifiers.season_id
  )
);
