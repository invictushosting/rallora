...
create policy "team_application_owner_insert" on public.rallora_team_applications for insert to authenticated with check(
 ...
 and exists(select 1 from public.seasons s join public.clubs c on c.id=s.club_id where s.id=season_id and c.id=club_id and s.status='active' and c.is_active)
);
...
begin
 ...
 if p_decision='approved' then
   ...
   values(p_division_id,trim(a.team_name),trim(a.captain_name),trim(a.partner_name),true) returning id into v_team;
   ...
   values(a.club_id,a.season_id,p_division_id,v_team,a.requested_by,a.captain_name,a.captain_phone,'approved',auth.uid(),now()) returning id into v_request;
 ...
 update public.rallora_team_applications set status=p_decision,division_id=case when p_decision='approved' then p_division_id else division_id end,reviewed_by=auth.uid(),reviewed_at=now(),created_team_id=v_team,updated_at=now() where id=a.id;
 return v_team;
end $$;
revoke all on function public.rallora_review_team_application(uuid,text,uuid) from public,anon;
grant execute on function public.rallora_review_team_application(uuid,text,uuid) to authenticated;
commit;