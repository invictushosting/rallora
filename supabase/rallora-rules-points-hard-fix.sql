-- Rallora: hard fix for rules points not applying to standings
...
-- Make sure all seasons belong to the active club if older data predates multi-club support.
...
begin
  ...
  if v_club_id is null then
    raise exception 'No active club found for this season.';
  end if;

  -- IMPORTANT:
  -- Prefer the season-specific rule row first. This prevents a club default row or old fallback row
  -- from overriding the points the organiser just saved for the active season.
  select
  ...
  order by
    case when cr.season_id = p_season_id then 0 else 1 end,
